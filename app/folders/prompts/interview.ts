...
/** 인터뷰 에이전트가 쓰는 함수 호출 툴 정의 */
export function buildInterviewToolset(): OpenAI.Chat.Completions.ChatCompletionTool[] {
  return [
    {
      type: "function",
      function: {
...
export function buildInterviewSystemPrompt(
  ...
  const { agentPersona, interviewGuide } = getCategoryPrompts(category.slug);
  const storyBlock =
    story.trim().length > 0 ? story.trim() : "(아직 없음 — 첫 대화입니다)";

  return [
    `${agentPersona} 당신의 임무는 정보를 수집하는 것이 아니라, 사용자의 기억과 감정을 존중하며 함께 머무는 것입니다. 자서전 한 챕터가 될 이야기를, 정해진 항목을 채우는 식이 아니라 자유롭고 따뜻한 대화로 천천히 길어 올려 주세요.`,
    "",
    ...
    "- 따뜻하고 정중하게, 천천히. 정중한 존댓말을 씁니다. 평가하거나 가르치려 들지 않습니다. '맞다/틀리다'가 없는 대화입니다.",
    "",
    "[분기 처리]",
...
/**
 * 대화창에 다시 들어온 사용자를 맞이하는 '재회 인사말' 시스템 프롬프트.
 * 저장된 이야기를 짧게 되짚어 주고 이어가도록 안내한다.
 */
export function buildRecapGreetingPrompt(
  category: CategoryConfig,
  story: string,
): string {
  const { agentPersona } = getCategoryPrompts(category.slug);
  return [
    agentPersona,
